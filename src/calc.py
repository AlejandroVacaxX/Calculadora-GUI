...
class CalcButton(ft.Button):
    expand: int = field(default_factory = lambda: 1) #type: ignore


@ft.control
class DigitButton(CalcButton):
    bgcolor: ft.Colors = ft.Colors.WHITE_24  #type: ignore
    color: ft.Colors = ft.Colors.WHITE      #type: ignore


@ft.control
class ActionButton(CalcButton):
    bgcolor: ft.Colors = ft.Colors.ORANGE #type: ignore
    color: ft.Colors = ft.Colors.WHITE #type: ignore


@ft.control
class ExtraActionButton(CalcButton):
    bgcolor: ft.Colors = ft.Colors.BLUE_GREY_100 #type: ignore
    color: ft.Colors = ft.Colors.BLACK #type: ignore
...
class CalculatorApp(ft.Container):
    def init(self):
        ...
        # this is the part where we build the layout
    ...
    # this function handles the click in button
    def button_clicked(self, e):
        ...
        if self.result.value == "Error" or data == "AC":
            ...
            self.result.value = self.calculate( #type: ignore
                self.operand1, float(self.result.value), self.operator # type: ignore
            )
            self.operator = data
            ...
            self.result.value = float(self.result.value) / 100 # type: ignore
            self.reset()
        ...
        self.update()
    # this function formats the number and remove the innecessary decimal part
    ...
    def calculate(self, operand1, operand2, operator):
        if operator == "+":
            ...
            if operand2 == 0:
                ...
                return self.format_number(operand1 / operand2)
    # we reset the calculator state
    def reset(self):
        self.operator = "+"
        self.operand1 = 0
        self.new_operand = True

# here we instantiate the app and run it
